...
jQuery ->
  ...
  validate_email = ()->
    if $("#email").val()
      ...
      val = $("#email").val();
      if reg.test(val)
        jQuery.post('/student/register/validate_uniqueness_email', {email: $("#email").val()},(data)->
          (
            if data != "true"
  ...
  change_faculty = () ->
    ...
    if faculty
      ...
      faculty = 0
    course = $("select#course :selected").val();
    if course
      course = course
    else
      course = 0
    jQuery.get('/student/register/update_group_select/' + faculty + '/' + course, (data)->($("#group").parent('p').html(data)));
