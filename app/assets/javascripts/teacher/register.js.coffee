...
jQuery ->
  ...
  validate_email = ()->
    if $("#email").val()
      ...
      val = $("#email").val();
      if reg.test(val)
        jQuery.post('/teacher/register/validate_uniqueness_email', {email: $("#email").val()},(data)->
          (
            if data != "true"
  ...
  change_faculty = () ->
    ...
    if faculty
      ...
      faculty = 0
    jQuery.get('/teacher/register/update_chair_select/' + faculty, (data)->($("#chair").parent('p').html(data)));
