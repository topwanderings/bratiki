class Teacher::PairController < ApplicationController
  before_filter :check_teacher
  def index
    @date1 =  Time.new(params[:year], params[:month], params[:day], 0, 0, 0, 0)
    @date2 = @date1 + 1.day
    @pairs = Pair.where(:date_time => @date1..@date2, :teacher_id => teacher_id)
  end
  ...
  def new
    if request.post?
      ...
      pair = Pair.new(:object => params[:object], :kind => params[:kind], :teacher_id => teacher_id, :date_time => date_time, :place => params[:place], :group => params[:group])
      if pair.save
        redirect_to({:controller => '/teacher/pair', :action => 'index', :day => params[:day], :month => params[:month], :year => params[:year]})
      else
